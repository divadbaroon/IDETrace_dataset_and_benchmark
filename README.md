...
├── datanotes.md                    # Deployment identification, D4 status, caveats
├── datasheet.md
├── croissant.json
└── manifest.yaml
```
...
Reported event counts exclude continuous pointer sampling (`MOUSE_MOVE`, emitted every 50 ms), which accounts for roughly three quarters of raw event volume. See `datanotes.md` before comparing raw file totals against the paper.
