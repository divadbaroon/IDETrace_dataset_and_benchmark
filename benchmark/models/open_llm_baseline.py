"""
LLM Baseline Evaluation for TutorTrace Benchmark (Open-Source via Ollama).

Evaluates open-source LLMs via Ollama on four tasks:
...
Usage:
  ...
  python3 benchmark/models/open_llm_baseline.py --model llama3.1:8b --tasks query_type
  python3 benchmark/models/open_llm_baseline.py --model qwen3.5:9b --tasks query_type
  python3 benchmark/models/open_llm_baseline.py --model deepseek-r1:8b --tasks query_type
"""
...
def run_multiclass_task(ollama_url, df, model, sample_size, task_name, label_col,
...
def main():
    parser = argparse.ArgumentParser(description='LLM Baseline Evaluation (Open-Source via Ollama)')
    ...
    parser.add_argument('--ollama-url', default='http://localhost:11434/v1', help='Ollama API base URL')
    parser.add_argument('--output', default='llm_results_open.json', help='Output JSON path')
    ...
    print("=" * 60)
    print("  TUTORTRACE LLM BASELINE (OPEN-SOURCE)")
    print("=" * 60)
    ...
    if 'query_type' in tasks_to_run and df_windows is not None:
        ...
        if len(df_qt) > 0:
            result = run_binary_task(
                args.ollama_url, df_qt, args.model, args.sample_size,
