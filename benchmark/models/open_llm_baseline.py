"""
Open-Source LLM Baseline Evaluation for TutorTrace Benchmark.

Evaluates open-source LLMs via Ollama on three tasks using the same test data as ML models:
...
Run all three back to back:
  python3 benchmark/models/open_llm_baseline.py --model qwen3.5:9b --output llm_results_qwen.json && \
  python3 benchmark/models/open_llm_baseline.py --model llama3.1:8b --output llm_results_llama.json && \
  python3 benchmark/models/open_llm_baseline.py --model deepseek-r1:8b --output llm_results_deepseek.json
"""
...
def parse_state(response_text):
    """Extract a behavioral state from LLM response, handling verbose outputs."""
    if response_text is None:
        return None
...
def run_next_state_task(client, df, model, sample_size):
    ...
    STATE_NAMES = {0: 'thinking', 1: 'implementing', 2: 'debugging', 3: 'seekingHelp', 4: 'testing'}
...
def main():
    parser = argparse.ArgumentParser(description='Open-Source LLM Baseline Evaluation')
    ...
    print("=" * 60)
    print("  TUTORTRACE OPEN-SOURCE LLM BASELINE")
    print("=" * 60)
