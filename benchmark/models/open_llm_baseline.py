...
Run only query type:
  python3 benchmark/models/open_llm_baseline.py --model llama3.1:8b --tasks query_type --output llm_results_llama31_qt.json
"""
...
def parse_query_type(response_text):
    ...
    text = strip_think_tags(response_text).lower().strip()
    if not text:
        return None
